...
void GVRET_Comm_Handler::processIncomingByte(uint8_t in_byte)
{
    ...
    case GET_COMMAND:
        ...
        case PROTO_TIME_SYNC:
            state = TIME_SYNC;
            step = 0;
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(1); // time sync
            txBuffer->sendByteToBuffer((uint8_t)(now & 0xFF));
            txBuffer->sendByteToBuffer((uint8_t)(now >> 8));
            txBuffer->sendByteToBuffer((uint8_t)(now >> 16));
            txBuffer->sendByteToBuffer((uint8_t)(now >> 24));
            break;
        case PROTO_DIG_INPUTS:
            ...
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(2); // digital inputs
            txBuffer->sendByteToBuffer(temp8);
        ...
        case PROTO_ANA_INPUTS:
            ...
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(3);
            txBuffer->sendByteToBuffer(temp16 & 0xFF);
            txBuffer->sendByteToBuffer(uint8_t(temp16 >> 8));
        ...
        case PROTO_GET_CANBUS_PARAMS:
            ...
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(6);
            txBuffer->sendByteToBuffer(settings.canSettings[0].enabled + ((unsigned char)settings.canSettings[0].listenOnly << 4));
            txBuffer->sendByteToBuffer(settings.canSettings[0].nomSpeed);
            txBuffer->sendByteToBuffer(settings.canSettings[0].nomSpeed >> 8);
            txBuffer->sendByteToBuffer(settings.canSettings[0].nomSpeed >> 16);
            txBuffer->sendByteToBuffer(settings.canSettings[0].nomSpeed >> 24);
            txBuffer->sendByteToBuffer(settings.canSettings[1].enabled + ((unsigned char)settings.canSettings[1].listenOnly << 4));
            txBuffer->sendByteToBuffer(settings.canSettings[1].nomSpeed);
            txBuffer->sendByteToBuffer(settings.canSettings[1].nomSpeed >> 8);
            txBuffer->sendByteToBuffer(settings.canSettings[1].nomSpeed >> 16);
            txBuffer->sendByteToBuffer(settings.canSettings[1].nomSpeed >> 24);
            state = IDLE;
            break;
        case PROTO_GET_DEV_INFO:
            // immediately return device information
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(7);
            txBuffer->sendByteToBuffer(CFG_BUILD_NUM & 0xFF);
            txBuffer->sendByteToBuffer(CFG_BUILD_NUM >> 8);
            txBuffer->sendByteToBuffer(0x20);
            txBuffer->sendByteToBuffer(0);
            txBuffer->sendByteToBuffer(0);
            txBuffer->sendByteToBuffer(0); // was single wire mode. Should be rethought for this board.
            state = IDLE;
            break;
        ...
        case PROTO_KEEPALIVE:
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(0x09);
            txBuffer->sendByteToBuffer(0xDE);
            txBuffer->sendByteToBuffer(0xAD);
            state = IDLE;
            break;
        ...
        case PROTO_GET_NUMBUSES:
            txBuffer->sendByteToBuffer(0xF1);
            txBuffer->sendByteToBuffer(12);
            txBuffer->sendByteToBuffer(SysSettings.numBuses);
            state = IDLE;
            break;
...
void GVRET_Comm_Handler::setBuffer(CommBuffer *buffer)
{
    txBuffer = buffer;
}

CommBuffer *GVRET_Comm_Handler::getBuffer()
{
    return txBuffer;
}